...
export default function AddPost() {
  ...
  const [dossierData, setDossierData] = useState({
    title: '', content: '', photoUrl: null, photoFile: null
  });

  const [currentFieldId, setCurrentFieldId] = useState('title');
  ...
  const isMultiline = (id) => ['content'].includes(id);
  ...
  const handlePublish = async () => {
    ...
    // Create multiparty form data structure requested by the backend route
    const formData = new FormData();
    formData.append("content", dossierData.content);
    if (dossierData.title) formData.append("title", dossierData.title);
    if (dossierData.photoFile) formData.append("photo", dossierData.photoFile);

    try {
      const response = await fetch("https://api.sicari.works/api/posts/add", {
        method: "POST",
        credentials: "include", // Inherit browser session block
        body: formData,
      });
      ...
      // Navigate out to newspaper overview.
      navigate("/feed");
    } catch (e) {
      console.error(e);
      alert("Error adding post. Please try again.");
      setLockState('idle'); // Rollback state.
    }
  };
      ...
      >
        ...
        <div className="td-paper-container">
          <div className="td-paper" ref={paperRef}>
            ...
            <div className="td-header">ADD NEW POST AMONG SICARIOS !</div>
          ...
          <div className="td-section">
            ...
            <div className="td-photo mt-[50px] inline-block" onClick={() => document.getElementById('photoUpload').click()}>
              ...
              {dossierData.photoUrl ? <img src={dossierData.photoUrl} alt="" className="max-w-[400px] border-[1px] border-black p-2 bg-[#eee4d1]" /> : <span>📎 ATTACH PHOTOGRAPH (OPTIONAL)</span>}
            </div>
          </div>
        </div>
        </div>

        {lockState === 'slamming' && <div className="td-slam-stamp">PUBLISHED</div>}
